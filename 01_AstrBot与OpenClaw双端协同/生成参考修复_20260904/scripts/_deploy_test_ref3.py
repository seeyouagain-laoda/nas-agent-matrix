...
launch = ("setsid env GEMINI_DEBUG_PORT=16002 "
          ...
          "--out <NAS_DATA_DIR>/memes/generated/test_ref3.png "
          "--no-launch > /tmp/ref_test3.log 2>&1 < /dev/null & echo LAUNCHED")
print("== launch =="); print(run(launch, t=15))
time.sleep(8)
print("== early log =="); print(run("cat /tmp/ref_test3.log 2>/dev/null | tail -15", t=15))
for i in range(22):
    log = run("cat /tmp/ref_test3.log 2>/dev/null", t=15)
    if "RESULT_JSON" in log:
        print("== DONE at poll %d ==" % i)
        for line in log.splitlines():
            if "REF_UPLOAD" in line or "RESULT_JSON" in line or "视口截图" in line:
                print(line)
        break
    if i % 3 == 0:
    ...
    print("== TIMEOUT =="); print(run("cat /tmp/ref_test3.log 2>/dev/null | tail -25", t=15))
print("== output file =="); print(run("ls -la <NAS_DATA_DIR>/memes/generated/test_ref3.png 2>/dev/null", t=15))
c.close()
